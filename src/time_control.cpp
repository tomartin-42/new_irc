...
{ 
	...
	time_control::get_time(&this->t_set_pollout);
}

time_t	time_control::get_t_ping() { return this->t_ping; }

void	time_control::set_t_ping(const time_t time) { this->t_ping = time; }
...
bool	time_control::get_kick() { return this->kick; }

time_t	time_control::get_time() { return std::time(NULL); }

void	time_control::get_time(time_t* var) { std::time(var); }
